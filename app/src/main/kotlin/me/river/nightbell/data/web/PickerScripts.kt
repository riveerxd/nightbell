package me.river.nightbell.data.web
...
object PickerScripts {
    ...
    val BOOTSTRAP: String = """
        (function(){
          if (window.__nightbellInstalled) { window.__nightbellRehighlight && window.__nightbellRehighlight(); return 'already'; }
          window.__nightbellInstalled = true;
          window.__nightbellPickMode = window.__nightbellPickMode || false;
          $HELPERS

          var style = document.createElement('style');
          style.setAttribute('data-nightbell','1');
          style.textContent = [
            '.__nightbell_hover{outline:2px dashed #6EE7FF !important;outline-offset:2px !important;',
            'background-color:rgba(110,231,255,0.10) !important;cursor:crosshair !important;}',
            '.__nightbell_picked{outline:3px solid #B388FF !important;outline-offset:2px !important;',
          ...
          function clearHover(){ if(hovered){ hovered.classList.remove('__nightbell_hover'); hovered = null; } }
          function setPicked(el){
            if(picked) picked.classList.remove('__nightbell_picked');
            picked = el;
            if(picked) picked.classList.add('__nightbell_picked');
          }
          window.__nightbellRehighlight = function(){
            if(window.__nightbellLastSelector){
              try {
                var el = document.querySelector(window.__nightbellLastSelector);
                if(el) setPicked(el);
              } catch(e){}
            }
          };
          window.__nightbellSetPickMode = function(on){
            window.__nightbellPickMode = !!on;
            if(!on) clearHover();
            document.documentElement.style.webkitUserSelect = on ? 'none' : '';
            return window.__nightbellPickMode;
          };
          window.__nightbellClear = function(){ setPicked(null); window.__nightbellLastSelector = null; clearHover(); };

          function describe(el){
            var sel = __pSelector(el);
            window.__nightbellLastSelector = sel;
          ...
          function onOver(e){
            if(!window.__nightbellPickMode) return;
            var el = e.target;
            if(!el || el.nodeType !== 1) return;
            if(el === hovered) return;
            clearHover();
            hovered = el;
            el.classList.add('__nightbell_hover');
          }
          ...
          document.addEventListener('mousedown', function(e){ if(window.__nightbellPickMode){ e.preventDefault(); e.stopPropagation(); } }, true);
    ...
    fun setPickMode(active: Boolean): String =
        "(function(){ return window.__nightbellSetPickMode ? window.__nightbellSetPickMode($active) : false; })();"

    const val CLEAR_SELECTION: String =
        "(function(){ if(window.__nightbellClear) window.__nightbellClear(); return true; })();"
