...
import me.river.nightbell.UrgentAlertActivity
...
class AlertCenter(private val context: Context) {
     ...
     * [me.river.nightbell.data.work.NightbellMonitorService] posts this as its own
     ...
     * [me.river.nightbell.domain.CheckerHealth].
     *
    ...
    fun notifyCheckerCrash(
        state: me.river.nightbell.domain.CheckerHealth.State,
        monitorName: String,
        policy: AlertPolicy,
        silent: Boolean,
        repeat: Boolean,
    ...
    fun urgentChannel(policy: AlertPolicy, respectRinger: Boolean = true): String {
        ...
        // change, so the 1.1.0-era `nightbell.urgent.*` channels on an existing
        ...
        if (manager.getNotificationChannel(id) != null) return id
        runCatching { manager.deleteNotificationChannel("nightbell.urgent.${style.name.lowercase()}") }
    ...
    companion object {
        private const val GROUP_DOWN = "nightbell.group.down"
        private const val GROUP_DEGRADED = "nightbell.group.degraded"
        private const val GROUP_RECOVERY = "nightbell.group.recovery"
        ...
        const val SERVICE_CHANNEL = "nightbell.service.strict"
        const val HEALTH_CHANNEL = "nightbell.health.checker"
         ...
         */
        const val URGENT_CHANNEL_V2 = "nightbell.urgent.v2"
