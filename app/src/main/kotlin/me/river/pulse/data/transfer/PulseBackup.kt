package me.river.pulse.data.transfer
...
data class PulseBackup(
    ...
    val snapshot: PulseSnapshot = PulseSnapshot(),
) {
    companion object {
...
sealed interface BackupError {
    ...
    data object Unreadable : BackupError {
        override val message: String = "That file isn't a Pulse backup"
    }

    /** Written by a build that knows a format this one does not. */
    data class TooNew(val format: Int) : BackupError {
        override val message: String =
            "That backup was written by a newer version of Pulse (format $format)"
    }
...
/**
 * Reads and writes [PulseBackup] documents.
 *
...
object BackupCodec {
     ...
     */
    fun decode(raw: String): Result<PulseBackup> {
        val backup = runCatching { strict.decodeFromString<PulseBackup>(raw) }
            .getOrElse { return Result.failure(BackupFailure(BackupError.Unreadable)) }
        if (backup.format > PulseBackup.FORMAT_VERSION) {
 ...
 * me.river.pulse.domain.LegacyCrashRepair] exists to undo. The down
 ...
 */
fun PulseBackup.toImportableSnapshot(): PulseSnapshot {
