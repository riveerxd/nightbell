...
class PulseE2ETest {
    ...
    fun setUp() {
        ...
        server = TinyHttpServer { request ->
            when {
                request.path.startsWith("/ok") -> TinyHttpServer.Response(body = "PULSE-OK")
    ...
    fun createsAndRunsAPassingStatusMonitor() {
        ...
        awaitTrue(description = "monitor stored") {
            runBlocking { Pulse.require().store.currentSnapshot().monitors.size } == 1
        }
        awaitTrue(description = "monitor reports UP") {
            runBlocking {
                Pulse.require().store.currentSnapshot().runtimes.values.firstOrNull()?.health == Health.UP
            }
        }
    ...
    fun failingStatusMonitorShowsAReadableFailure() {
        ...
        awaitTrue(description = "monitor reports DOWN") {
            runBlocking {
                Pulse.require().store.currentSnapshot().runtimes.values.firstOrNull()?.health == Health.DOWN
            }
        }
        composeRule.waitForIdle()
        ...
        val runtime = runBlocking { Pulse.require().store.currentSnapshot().runtimes.values.first() }
    ...
    fun bodyExpectationDrivesPassAndFail() {
        launchApp()
        // Passing: the /ok endpoint really does contain PULSE-OK.
        createMonitor(
            name = "Body match",
            url = server.url("/ok"),
            bodyContains = "PULSE-OK",
        )
    ...
    fun detailScreenShowsHistoryAndConfiguration() {
        ...
        awaitTrue(description = "first check completed") {
            runBlocking {
                (Pulse.require().store.currentSnapshot().runtimes.values.firstOrNull()?.samples?.size ?: 0) > 0
            }
        }
        composeRule.waitForIdle()
        ...
        awaitTrue(description = "second sample recorded") {
            runBlocking {
                (Pulse.require().store.currentSnapshot().runtimes.values.firstOrNull()?.samples?.size ?: 0) >= 2
            }
        }
    }
    ...
    fun monitorsSurviveAnAppRestart() {
        ...
        val stored = runBlocking { Pulse.require().store.currentSnapshot() }
    ...
    fun settingsExposeAlertSoundVibrationAndEscalation() {
        ...
        awaitTrue(description = "vibration style persisted") {
            runBlocking {
                Pulse.require().store.currentSnapshot().settings.defaultAlert.vibrationStyle.name == "SOS"
            }
        }
        ...
        awaitTrue(description = "sound choice persisted") {
            runBlocking {
                Pulse.require().store.currentSnapshot().settings.defaultAlert.sound.name == "ALARM"
            }
        }
        ...
        awaitTrue(description = "repeat toggle persisted") {
            runBlocking { Pulse.require().store.currentSnapshot().settings.defaultAlert.repeatEnabled }
        }
        ...
        awaitTrue(description = "quiet hours persisted") {
            runBlocking { Pulse.require().store.currentSnapshot().settings.defaultAlert.quietHoursEnabled }
        }
    ...
    fun setupWizardBlocksInvalidInputAndExplainsWhy() {
        ...
        val storedBefore = runBlocking { Pulse.require().store.currentSnapshot().monitors.size }
        composeRule.onNodeWithText("Continue").performClick()
        composeRule.waitForIdle()
        composeRule.onNodeWithContentDescription("URL").assertIsDisplayed()
        assertEquals(storedBefore, runBlocking { Pulse.require().store.currentSnapshot().monitors.size })
    }
